...
if [[ "${EUID}" -ne 0 ]]; then
    echo "[ERROR] Please run this script with sudo (for example: sudo ./2_run_NSA_capture.sh)" >&2
    exit 1
fi
...
QLOG_CFG="${QLOG_DIR}/conf/5GNR_LTE_CN_V11.cfg"
QCOM_CFG="${QCOM_DIR}/configs/qcm-auto.conf"
...
(
    cd "${QCOM_DIR}"
    ${SUDO} ./out/quectel-cm-test -q -c ./configs/qcm-auto.conf
) &
QCOM_PID=$!
