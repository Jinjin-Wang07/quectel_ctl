...
usage() {
  ...
  echo "- qmdl2-log-dir defaults to quectel_qlog/log or quectel_qlog/out/logs"
...
echo "[1/3] Copying qmdl2 logs from: $LOG_DIR"
...
for src in "${QMDL_FILES[@]}"; do
  ...
  copied_qmdl="${OUTPUT_DIR}/${base_name}"
  ...
  if scat -t qc -d "$copied_qmdl" -F "$pcap_out" >"$scat_log" 2>&1; then
